...
class FutureMCJEIPlugin : IModPlugin {
    ...
    override fun register(registry: IModRegistry) {
        ...
        if (villageAndPillage.smoker) {
            registry.handleRecipes(FurnaceRecipe::class.java, ::SmokerRecipeWrapper, SmokerRecipeCategory.NAME)
            registry.addRecipes(SmokerRecipes.recipes, SmokerRecipeCategory.NAME)
            registry.addRecipeClickArea(
                GuiFurnaceAdvanced.BlastFurnace::class.java,
                78,
                32,
                28,
                23,
                BlastFurnaceRecipeCategory.NAME,
                VanillaRecipeCategoryUid.FUEL
            )
            recipeTransferRegistry.addRecipeTransferHandler(
                ContainerFurnaceAdvanced::class.java,
                SmokerRecipeCategory.NAME,
                0,
                1,
                3,
                36
        ...
        if (villageAndPillage.blastFurnace) {
            ...
            registry.addRecipeClickArea(
                GuiFurnaceAdvanced.Smoker::class.java,
                78,
                32,
                28,
                23,
        ...
        if (villageAndPillage.stonecutter.enabled) {
            registry.handleRecipes(
                StonecutterRecipeWrapper::class.java,
                { it },
            ...
            if (villageAndPillage.stonecutter.recipeButton) {
                registry.addRecipeClickArea(GuiStonecutter::class.java, 143, 8, 16, 16, StonecutterRecipeCategory.NAME)
            }
