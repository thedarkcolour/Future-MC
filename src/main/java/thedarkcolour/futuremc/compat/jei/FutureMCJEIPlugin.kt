...
class FutureMCJEIPlugin : IModPlugin {
    ...
    override fun register(registry: IModRegistry) {
        ...
        if (villageAndPillage.smoker) {
            registerAdvancedFurnace(helpers, GuiFurnaceAdvanced.Smoker::class.java, SmokerRecipes, AdvancedFurnaceRecipeCategory.SMOKING, SMOKER, registry, recipeTransferRegistry)
        }
        if (villageAndPillage.blastFurnace) {
            registerAdvancedFurnace(helpers, GuiFurnaceAdvanced.BlastFurnace::class.java, BlastFurnaceRecipes, AdvancedFurnaceRecipeCategory.BLASTING, BLAST_FURNACE, registry, recipeTransferRegistry)
        }
        if (villageAndPillage.campfire.enabled && villageAndPillage.campfire.functionality) {
            registry.handleRecipes(CampfireRecipe::class.java, { CampfireRecipeWrapper(helpers, it) }, CampfireRecipeCategory.NAME)
        ...
        if (villageAndPillage.stonecutter.enabled && villageAndPillage.stonecutter.functionality) {
            registry.handleRecipes(SimpleRecipe::class.java, { SimpleRecipeWrapper(helpers, it) }, StonecutterRecipeCategory.NAME)
        ...
        if (villageAndPillage.smithingTable.enabled && villageAndPillage.smithingTable.functionality) {
            registry.handleRecipes(SmithingRecipe::class.java, { SmithingRecipeWrapper(helpers, it) }, SmithingRecipeCategory.NAME)
        ...
        if (villageAndPillage.composter) {
            registry.handleRecipes(Object2ByteMap.Entry::class.java as Class<Object2ByteMap.Entry<Ingredient>>, { ComposterRecipeWrapper(helpers, it) }, ComposterRecipeCategory.NAME)
    ...
    private fun registerAdvancedFurnace(
        ...
        recipes: Recipes<SimpleRecipe>,
        title: String,
        block: Block,
        registry: IModRegistry,
        recipeTransferRegistry: IRecipeTransferRegistry
    ) {
        registry.handleRecipes(SimpleRecipe::class.java, { SimpleRecipeWrapper(helpers, it) }, title)
        registry.addRecipes(recipes.recipes, title)
