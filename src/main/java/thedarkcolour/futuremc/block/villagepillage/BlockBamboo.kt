...
open class BlockBamboo(properties: Properties) : FBlock(properties), IGrowable {
    init {
        defaultState = defaultState.withProperty(THICK, false).withProperty(LEAVES, EnumLeaves.NO_LEAVES).withProperty(
            MATURE, false)
    }
    ...
    override fun isSideSolid(state: IBlockState, worldIn: IBlockAccess, pos: BlockPos, side: EnumFacing) = false
    ...
    companion object {
        ...
        private val TIGHT_THICK_AABB = cube(6.5, 0.0, 6.5, 9.5, 16.0, 9.5)
        private val TIGHT_THIN_AABB = cube(7.0, 0.0, 7.0, 9.0, 16.0, 9.0)
        private val NORMAL_AABB = cube(5.0, 0.0, 5.0, 11.0, 16.0, 11.0)
        private val LEAVES_AABB = cube(3.0, 0.0, 3.0, 13.0, 16.0, 13.0)
    }
}