...
class SmithingContainer(type: ContainerType<*>?, id: Int, playerInv: PlayerInventory, private val worldPos: IWorldPosCallable) : Container(type, id) {
    ...
    fun canTakeOutput(): Boolean {
        return true
    }

    fun onTakeOutput(stack: ItemStack): ItemStack {
        worldPos.consume { worldIn, pos ->
            if (!worldIn.isRemote) {
                val stack1 = input.getStackFrom(1)
                stack1.shrink(recipe!!.materialCost)
