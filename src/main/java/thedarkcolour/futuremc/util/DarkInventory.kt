...
open class DarkInventory constructor(size: Int, defaultName: String? = null) : IItemHandlerModifiable,
        INBTSerializable<CompoundNBT>, Iterable<ItemStack> {
    ...
    //override fun getStackFrom(slot: Int) = stacks[slot]
     ...
     * Only works if the Slot is [DarkInventorySlot]
     * @see DarkInventorySlot.canTakeStack
     */
    ...
    override fun deserializeNBT(tag: CompoundNBT) {
        ...
        for (i in 0 until tagList.size) {
            ...
            if (slot >= 0 && slot < stacks.size) {
                val stack = ItemStack.read(itemTags)
                stacks[slot] = stack
            }
        }
    }
