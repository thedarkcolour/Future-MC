...
import thedarkcolour.futuremc.capability.SwimmingProvider;
...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.getHealth() > 0.0F && !this.isSpectator()
                // ADDED
                && !getCapability(SwimmingProvider.SWIMMING_CAP, null).isSwimming()
                //
        )
        {
    ...
    public void travel(float strafe, float vertical, float forward)
    {
        ...
        // ADDED
        if (getCapability(SwimmingProvider.SWIMMING_CAP, null).isSwimming() && !isRiding()) {
            double d3 = this.getLookVec().y;
            double d4 = d3 < -0.2D ? 0.085D : 0.06D;
            IBlockState state = world.getBlockState(new BlockPos(posX, posY + 1.0D - 0.1D, posZ));
            if (d3 <= 0.0D || this.isJumping || state.getBlock().getMaterial(state).isLiquid()) {
                motionY = (d3 - motionY) * d4;
            }
        }
        //
