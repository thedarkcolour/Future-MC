...
class HugeFungusFeature(configFactory: (Dynamic<*>) -> HugeFungusFeatureConfig) : Feature<HugeFungusFeatureConfig>(configFactory) {
    ...
    private fun tryGenerateVines(worldIn: IWorld, rand: Random, pos: BlockPos, state: BlockState, isCrimson: Boolean) {
    ...
    private fun generateVines(pos: BlockPos, worldIn: IWorld, rand: Random) {
        val mutable = Mutable(pos).move(Direction.DOWN)
        ...
        if (worldIn.isAirBlock(mutable)) {
            ...
            WeepingVinesFeature.placeVine(worldIn, rand, mutable, length, minAge = 23, maxAge = 25)
        }
    }

    private fun generateHatBlock(worldIn: IWorld, rand: Random, config: HugeFungusFeatureConfig, mutable: BlockPos, decorationChance: Float, generationChance: Float, vineChance: Float) {
