...
class WeepingVinesFeature(configFactory: (Dynamic<*>) -> NoFeatureConfig) : Feature<NoFeatureConfig>(configFactory) {
    override fun place(worldIn: IWorld, generator: ChunkGenerator<*>, rand: Random, pos: BlockPos, config: NoFeatureConfig): Boolean {
    ...
    companion object {
        ...
        private fun placeWartBlocksInArea(worldIn: IWorld, rand: Random, pos: BlockPos) {
            ...
            for (i in 0 until 200) {
                mutable1.setPos(pos).move(rand.nextInt(6) - rand.nextInt(6), rand.nextInt(2) - rand.nextInt(5), rand.nextInt(6) - rand.nextInt(6))
        ...
        private fun placeVinesInArea(worldIn: IWorld, rand: Random, pos: BlockPos) {
            ...
            for (l in 0 until 100) {
                ...
                if (worldIn.isAirBlock(mutable)) {
                    val block = worldIn.getBlockState(mutable.up()).block
                    if (block == Blocks.NETHERRACK || block == Blocks.NETHER_WART_BLOCK) {
