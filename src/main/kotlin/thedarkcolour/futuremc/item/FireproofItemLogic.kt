...
object FireproofItemLogic : ItemEntity(null, 0.0, 0.0, 0.0) {
        ...
        if (entity.item.isEmpty) {
            ...
            if (!entity.onGround || horizontalMag(entity.motion) > 1.0E-5f.toDouble() || (entity.ticksExisted + entity.entityId) % 4 == 0) {
                ...
                if (entity.noClip) {
                    ...
                    val vec3d0 = entity.getAllowedMovement(entity.motion)
                    ...
                    try {
                        ...
                        val report = CrashReport.makeCrashReport(throwable, "Checking entity block collision")
                        val reportCategory = report.makeCategory("Entity being checked for collision")
                        entity.fillCrashReport(reportCategory)
                        throw ReportedException(report)
                    }

                    val velocityMultiplier = entity.velocityMultiplier.toDouble()
                    entity.motion = entity.motion.mul(velocityMultiplier, 1.0, velocityMultiplier)
